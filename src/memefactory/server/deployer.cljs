...
(defn deploy [{:keys [:write? :initial-registry-params :transfer-dank-token-to-accounts]
  ...
  (let [accounts (web3-eth/accounts @web3)
        deploy-opts (merge {:from (first accounts)
                            ...
                            :meme-auction-cut-collector (first accounts)
                            :meme-auction-cut 0}
                           deploy-opts)]
    ...
    (ds-auth/set-authority :district-config (contract-address :ds-guard))
    ...
    (ds-auth/set-authority :meme-auction-factory-fwd (contract-address :ds-guard))
